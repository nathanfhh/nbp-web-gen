...
export function useSlidesApi() {
  ...
  /**
   * Dispatch a structured-JSON LLM call with thinking to the right provider.
  ...
  const generateStructuredJson = async ({
    ...
    // Gemini: stream with native thinking, with Free Tier → Paid fallback
...
Write all descriptions in English.`
    ...
    try {
      const textResponse = await generateStructuredJson({
        model,
