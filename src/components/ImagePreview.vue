...
<template>
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 pb-4">
      ...
      >
        ...
        <button
          ...
          class="absolute top-2 right-2 p-2 rounded-lg bg-black/50 text-white/80 hover:bg-black/70 hover:text-text-primary transition-all"
  ...
  <div v-else class="flex flex-col items-center justify-center py-16 text-center">
    ...
    <p class="text-text-muted">{{ $t('preview.empty') }}</p>
    <p class="text-sm text-text-muted mt-1">{{ $t('preview.emptyHint') }}</p>
  </div>
</template>
