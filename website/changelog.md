...
### 修復
- **逐字稿面板**: 修正雙人旁白在中文句號（。）後直接接講者名稱時無法正確分段的問題，導致整段文字擠在一起
...
### 新功能
- **MP4 匯出**: 新增旁白語速控制（1x–4x），使用自製 WSOLA 演算法保持音調不變。提供滑桿、數值輸入與快速預設按鈕（1/1.25/1.5/1.75/2/3x），設定自動記住
...
### 新功能
- **簡報旁白**: 新增 Lightbox 逐字稿浮動面板，支援拖曳移動、右上角拖曳調整大小，位置與尺寸自動記住。雙人模式以不同顏色標示講者，切換頁面時自動捲動至頂部。可透過工具列按鈕或 `T` 鍵切換顯示

### 修復
- **燈箱**: 修正拖曳或調整逐字稿面板大小時會意外觸發關閉燈箱的問題
...
### 新功能
- **文字模型**: 新增 Gemini 3.1 Pro 模型選項（簡報分析、內容拆分、旁白腳本、角色萃取）

### 修復
- **Agent 模式**: 修正 Agent 聊天模型被錯誤連結至通用預設值的問題，確保僅使用支援程式碼執行的 Flash 模型

### 重構
- **模型定義**: 將 Text LLM 模型列表統一至 `TEXT_MODELS` 常數（Single Source of Truth），消除 4 處重複定義與 8 處硬編碼預設值
